...
class TrainOptions:
    ...
    def initialize(self):
        ...
        self.parser.add_argument('--mask_dir', type=str, default='', help="directory where all masks are stored")

        self.parser.add_argument('--train_file_path', type=str, default='',
        ...
        self.parser.add_argument('--checkpoints_dir', type=str, default='training_checkpoints',
                                 help='here models are saved during training')
        self.parser.add_argument('--pretrained_model_dir', type=str, default='',
                                 help='pretrained model are provided here')
        ...
        self.parser.add_argument('--random_mask', type=int, default=0,
                                 help='0 -> Center 128 * 128 mask, 1 -> random mask')
        self.parser.add_argument('--random_mask_type', type=str, default='irregular_mask',
                                 help='options - irregular_mask and random_rect')
        self.parser.add_argument('--incremental_training', type=int, default=1,
                                 help='1 -> using incremental training, 0 -> not using incremental training')
    ...
    def parse(self):
        ...
        assert self.opt.random_mask_type in ['irregular_mask', 'random_rect', 'predefined']
        assert self.opt.incremental_training in [0, 1]

        if self.opt.random_mask == 2:
          assert self.opt.mask_dir != ''
        ...
        if self.opt.random_mask == 0:
            self.opt.model_folder += '_center_mask'
        elif self.opt.random_mask == 1:
            self.opt.model_folder += '_random_mask'
        else:
            self.opt.model_folder += '_predefined_mask'
        ...
        self.opt.checkpoint_saving_dir = os.path.join(self.opt.base_dir, self.opt.model_folder,
